...
# rename OOS rom scripts (which reset the governor tunables regularly)
	if [ -f /system/etc/msm8994_hmp.sh ] || [ -f /system/etc/msm8994_tune.sh ]; then
		mount -o remount,rw -t ext4 $SYSTEM_DEVICE /system
		mv /system/etc/msm8994_hmp.sh /system/etc/msm8994_hmp.sh.bak
		mv /system/etc/msm8994_tune.sh /system/etc/msm8994_tune.sh.bak
		busybox sync
		mount -o remount,ro -t ext4 $SYSTEM_DEVICE /system
		echo $(date) "OOS rom scripts renamed"  >> $BOEFFLA_LOGFILE
	fi
...
# execute OOS rom scripts one time only now
	...
	/system/bin/sh /system/etc/msm8994_tune.sh.bak
	echo $(date) "Renamed OOS rom scripts executed"  >> $BOEFFLA_LOGFILE
