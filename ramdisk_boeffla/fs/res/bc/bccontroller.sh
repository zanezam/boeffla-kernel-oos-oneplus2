...
#
# *******************************
# Oneplus 2 OxygenOS 2.x.x version
#
# V0.1
# *******************************
...
KERNEL_SPECS="oneplus2;oos;oos2xx;http://boeffla.df-kunde.de/oneplus2/boeffla-kernel-oos/"
...
if [ "lov_cpu_hotplug_profiles_2" == "$1" ]; then
	echo "min=2, max=4 (default);min=2, max=4 (non-stock);min=0, max=4;min=0, max=3;min=0, max=2;min=0, max=1;min=0, max=0;min=1, max=4;min=1, max=3;min=1, max=2;min=1, max=1;min=2, max=3;min=2, max=2;min=3, max=4;min=3, max=3;min=4, max=4"
	exit 0
fi
...
if [ "apply_nfs" == "$1" ]; then
	if [ "1" == "$2" ]; then
		...
		insmod $LIBPATH/dns_resolver.ko
		insmod $LIBPATH/nfs.ko
		insmod $LIBPATH/nfsv3.ko
		insmod $LIBPATH/nfsv4.ko
	fi
